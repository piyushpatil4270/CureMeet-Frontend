...
const userType=localStorage.getItem("usertype")
  const fetchUser = async () => {
    try {
      const res = await axios.post('http://localhost:5500/auth/doctor/profile', { userId:userId,date:moment.utc().toDate() },{headers:{"Authorization":userToken,"userType":userType}});
  ...
  const fetchData=async()=>{
    try {
      const res=await axios.post("http://localhost:5500/appointments/doctor/monthlyData",{},{headers:{"Authorization":userToken,"userType":userType}})
  ...
  const createSlots=async()=>{
    try {
      const res=await axios.get("http://localhost:5500/slots/createSlots")
