...
const DApp = () => {
  ...
  const fetchAppointment = async () => {
    try {
      const res = await axios.post("http://localhost:5500/appointments/doctor/appointment", { appointmentId: appointmentId }, { headers: { "Authorization": userToken, "userType": userType } });
  ...
  const appointmentCompleted = async () => {
    try {
      const res = await axios.post("http://localhost:5500/appointments/successful", { appointmentId: appointmentId, presc: prescriptions, patientId: appointment.patient.id, doctorId: appointment.doctorId }, { headers: { "Authorization": userToken, "userType": userType } });
      if (res.status === 202) {
  ...
  const userDocs = async () => {
    if (appointment) {
      try {
        const res = await axios.post("http://localhost:5500/documents/getPatientDocuments ", { userId: appointment.patient.id }, { headers: { "Authorization": userToken, "userType": userType } });
